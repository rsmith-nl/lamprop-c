...
% Last modified: 2026-02-20T17:14:20+0100
...
\usepackage[bookmarks,pdfborder={0 0 0},colorlinks=true]{hyperref}
...
The \textbf{\texttt{f:}}-line contains a definition of a fiber. The parser
...
The \textbf{\texttt{r:}}-line contains a definition of a resin. Like with the
...
The \textbf{\texttt{t:}} line starts a new laminate. It only contains the name which
identifies the laminate. This name must be unique within the current input
files. It may contain spaces.

The \textbf{\texttt{m:}} line chooses a resin for the laminate. It must appear after
...
A \textbf{\texttt{c:}} line describes a comment. The comment should appear before an
\texttt{l:} line and applies to that lamina.

The \textbf{\texttt{l:}} line defines a single layer (lamina) in the laminate. It must be
...
The last line in a laminate definition can be an \textbf{\texttt{s:}} line, which stands
for ``symmetry''. This signifies that all the lamina before it are to be added
again in reverse order, making a symmetric laminate stack. A \texttt{s:} line in any
...
This uses about 1\% of the memory allocated by the program.
Calculating the properties of that laminate took approximately
