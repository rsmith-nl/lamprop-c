...
%
% Copyright © 2026 R.F. Smith <rsmith@xs4all.nl>. All rights reserved.
% Created: 2026-02-01T19:52:00+01:00
% Last modified: 2026-05-07T20:41:38+0200
...
\usepackage[bookmarks,pdfborder={0 0 0},colorlinks=true,citecolor=blue]{hyperref}
...
The generally hard to obtain transverse fiber properties
...
In this manual, UNIX-like systems like Linux, FreeBSD, Open BSD, NetBSD and macOS will be
referred to as “POSIX” for brevity.


\chapter{Requirements} % {{{2

To build this program you will need a C compiler that supports the C11
standard.
The build has been tested with \texttt{clang} version 19 and \texttt{gcc}
version 15.

Using the \texttt{make} program is optional but recommended.
Both GNU \texttt{make} and BSD \texttt{make} have been shown to work.

\section{POSIX}

On a POSIX system, the program can be built by simply invoking \texttt{make}.
It will use the C-compiler that the platform comes with.
This will usually be either \texttt{clang} or \texttt{gcc}.

\section{Ms-windows}

The only tested build method on ms-windows is using
\href{https://github.com/skeeto/w64devkit}{w64devkit}.
Open \texttt{w64devkit}, change to the directory where you have stored the source,
and issue the command \texttt{make~-f~Makefile.win32}.


\chapter{Using the program} % {{{1

The program does not \emph{require} installation to run. Only the binary is
required.

For convenience you can copy the binary to a location mentioned in the
environment variable \texttt{PATH} (POSIX) or \texttt{Path} (ms-windows).


\texttt{lamprop} processes input files with laminate descriptions.
These files are written in plain text, and have the extension \texttt{lam}.
The format of these files is documented in the next chapter.

The \texttt{lamprop} program runs on the command line.

\section{POSIX}

Assuming that \texttt{lamprop} in installed in your \texttt{PATH}, simply call
it as \texttt{lamprop} from your shell, X11 or Wayland terminal emulator.

If the location is not in your \texttt{PATH}, call the program with its
location:
\verb|~/.local/bin/lamprop -h|
.

\section{Ms-windows}

To run \texttt{lamprop.exe}, open the folder where the \texttt{lam} file you
want to process resides in the windows explorer.
Right-click in that folder, and select “Open in Windows Terminal”, see
\fref{fig:open-in-windows-terminal}.

\begin{figure}[!htbp]
  \centerline{\includegraphics{images/open-in-windows-terminal}}
  \caption{\label{fig:open-in-windows-terminal}Windows terminal in context
  menu}
\end{figure}

To run \texttt{lamprop.exe}, you have to specify the complete path where the
program is located, unless it is stored in a location that is located in the
\texttt{Path} environment variable.
When there are spaces in the path, the whole path has to be enclosed in double
quotation marks, and it has to be preceded by an ampersand, 
%see
for example:
\verb| & "C:\users\John Doe\lamprop\lamprop.exe" -h|
.

Note that this program uses \textsc{posix}-style options. Short
options starting with “\texttt{-}” and long options starting with “\texttt{--}”.

\section{Using the command-line program} % {{{2

The command \texttt{lamrop -h} produces the following overview of the options.

\lstinputlisting[style=plain]{data/help-output.txt}
...
appropriate arguments. \textsc{rtf} output (for inclusion in word
processor documents) has been removed. Since most word processors can read
\textsc{html}, use that instead.

By default, it produces all possible output, in text format.
Often we are only interested in the engineering properties.
In that case, use the \texttt{-e} option.

\chapter{The lamprop file format} % {{{2

The file format is very simple. Functional lines have either \textbf{f},
\textbf{r}, \textbf{t}, \textbf{m}, \textbf{l}, \textbf{c} or \textbf{s} as
the first non white space character. This character must immediately be
followed by a colon \textbf{:}. All other lines are seen as comments and
disregarded.
The information on the functional lines varies per type.
The different items on each lines should be separated by whitespace.
...
The \textbf{f:}-line contains a definition of a fiber. The parser
...
So the factor $\xi$ was reduced from 2 to 1.5 conform
\citet{Giner:2014}

The \textbf{r:}-line contains a definition of a resin. Like with the
...
The \textbf{t:} line starts a new laminate. It only contains the name which
identifies the laminate. This name must be unique within the current input
files. It may contain spaces.

The \textbf{m:} line chooses a resin for the laminate. It must appear
\emph{after} a \textbf{t:} line, and \emph{before} the \textbf{l:} lines. It
must contain the following values, separated by white space:
\begin{description}
    ...
    \item[$name$] The name of the resin to use. This must have been previously
        declared with an \textbf{r:}-line.
\end{description}

A \textbf{c:} line describes a comment. The comment should appear before an
\textbf{l:} line and applies to that lamina.

The \textbf{l:} line defines a single layer (lamina) in the laminate. It must be
preceded by a \textbf{t:} and a \textbf{m:} line. It must contain the following values,
...
\begin{description}
    ...
    \item[$name$] The name of the fiber used in this layer. This fiber must have been
        declared previously with an \textbf{f:} line.
\end{description}

The last line in a laminate definition can be an \textbf{s:} line, which stands
for ``symmetry''. This signifies that all the lamina before it are to be added
again in reverse order, making a symmetric laminate stack. A \textbf{s:} line in any
...
m: 0.5 Hyer's resin
c: 200 g/m2 plain weave
l: 100  0 Hyer's carbon fiber
l: 100 90 Hyer's carbon fiber
s:
\end{lstlisting}

At startup, \texttt{lamprop} reserves a 32 MiB block of virtual memory using
\texttt{mmap} (POSIX) or \texttt{VirtualAlloc} (ms-windows).
Only memory that is actually \emph{used} is allocated.
The author has personally used laminates with up to 250 layers.
This uses about 1\% of the reserved memory.
So in normal practice, memory is not a bottleneck of this program.
Calculating the properties of that 250-layer laminate took approximately
\SI{0.005}{s} on a machine with an Intel Core2 Q9300 running FreeBSD.

Interspersed between the \textbf{l:} lines (and before the \textbf{s:} line)
there can be \textbf{c:} lines.
These are comments about the lay-up, that will be inserted into the output.
Their use is to signify that the \textbf{l:} lines following a comment are
part of a subassembly such as a woven fabric or a non-crimp fabric.

For example:
\lstinputlisting[style=plain]{data/atlac430.lam}

This will yield the output shown in \trefpi{tab:atlac430-combi}.

\input{data/atlac430.tex}
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lrrrrl}% l,c,r
    ...
    \midrule
      ...
      Torayca T300 & 230000 & 0.27 & -0.41e-6 & 1.76 & carbon\\
...
\section{Generic materials}

Lamprop has a number of built-in generic materials.
Built-in means that you can use them in laminates without having to define
them.
The generic fibers are shown in \trefpi{tb:generic-fibers}.
The generic resins are shown in \trefpi{tb:generic-resins}.

\begin{table}[!htbp]
  \centering
  \caption{\label{tb:generic-fibers}Generic fibers}
  \begin{tabular}{lrrrr}% l,c,r
      Name & $E_1$ & $\nu_{12}$ & $\alpha_1$ & $\rho$\\
      & [\si{MPa}] & [-] & [\si{K^{-1}}] & [\si{g/cm^3}]\\
    \midrule
      generic-e-glass & 73000 & 0.22 & 5.3e-6 & 2.60\\
      generic-carbon & 230000 & 0.27 & -0.38e-6 & 1.80\\
      generic-aramid49 & 124000 & 0.36 & -4.9e-6 & 1.44\\
  \end{tabular}
\end{table}

\begin{table}[!htbp]
  \centering
  \caption{\label{tb:generic-resins}Generic resins}
  \begin{tabular}{lrrrr}% l,c,r
      Name & $E$ & $\nu$ & $\alpha$ & $\rho$\\
      & [\si{MPa}] & [-] & [\si{K^{-1}}] & [\si{g/cm^3}]\\
    \midrule
      generic-epoxy & 2900 & 0.29 & 40e-6 & 1.15\\
      generic-polyester & 4000 & 0.36 & 40e-6 & 1.20\\
      generic-vinylester & 3500 & 0.36 & 51.5e-6 & 1.10\\
  \end{tabular}
\end{table}
...
The ABD and H matrix equations in \tref{tab:quasi-isotropic-mat-2} basically show the
...
number of independent material constants from 81 to 21.
...
\emph{n}\textdegree{} there is also a layer with the same area weight at \emph{-n}\textdegree.
...
Classical laminate theory strictly speaking is only valid for stacks of
...
content, the degree of cure of the resin, the strength of the adhesion between
matrix and fibers and errors in cutting or placing the fibers.
These are determined by the type of production process used and the craftsmanship
